...
// AUTO-MIGRATION: Ensure excess_amount column exists
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT excess_amount FROM payments LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            console.log('Migrating: Adding excess_amount column...');
            await conn.query("ALTER TABLE payments ADD COLUMN excess_amount DECIMAL(10,2) DEFAULT 0.00");
        }
    } finally {
        conn.release();
    }
...
// AUTO-MIGRATION: Combo Support for Products
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT is_combo FROM products LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            console.log('Migrating: Adding combo columns to products...');
            try {
                await conn.query("ALTER TABLE products ADD COLUMN is_combo BOOLEAN DEFAULT FALSE");
                await conn.query("ALTER TABLE products ADD COLUMN combo_definition JSON DEFAULT NULL");
                console.log('Added combo columns to products.');
            } catch (alterErr) {
                console.error('Failed to add combo columns to products:', alterErr);
            }
        }
    } finally {
        conn.release();
...
// AUTO-MIGRATION: show_in_kds for Products (Skip KDS toggle)
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT show_in_kds FROM products LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            console.log('Migrating: Adding show_in_kds to products...');
            try {
                await conn.query("ALTER TABLE products ADD COLUMN show_in_kds TINYINT(1) DEFAULT 1");
                console.log('Added show_in_kds to products.');
            } catch (alterErr) {
                console.error('Failed to add show_in_kds to products:', alterErr);
            }
        }
    } finally {
        conn.release();
...
// AUTO-MIGRATION: description for Products (Customer Portal)
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT description FROM products LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            ...
            try {
                ...
                console.error('Failed to add description to products:', alterErr);
            }
        }
    } finally {
        conn.release();
...
// AUTO-MIGRATION: image_url for Products (Customer Portal)
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT image_url FROM products LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            ...
            try {
                ...
                console.error('Failed to add image_url to products:', alterErr);
            }
        }
    } finally {
        conn.release();
...
// AUTO-MIGRATION: Combo Selections for Order Items
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT combo_selections FROM order_items LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            console.log('Migrating: Adding combo_selections to order_items...');
            try {
                await conn.query("ALTER TABLE order_items ADD COLUMN combo_selections JSON DEFAULT NULL");
                console.log('Added combo_selections to order_items.');
            } catch (alterErr) {
                console.error('Failed to add combo_selections:', alterErr);
            }
        }
    } finally {
        conn.release();
...
// AUTO-MIGRATION: Ensure chef column exists in orders
(async () => {
    const conn = await pool.getConnection();
    try {
        ...
        if (e.code === 'ER_BAD_FIELD_ERROR') {
            console.log('Migrating: Adding chef column to orders...');
            try {
                await conn.query("ALTER TABLE orders ADD COLUMN chef VARCHAR(100) DEFAULT NULL AFTER waiter_id");
                console.log('Added chef column to orders.');
            } catch (alterErr) {
                console.error('Failed to add chef column:', alterErr);
            }
        }
    } finally {
        conn.release();
...
(async () => {
    ...
    try {
        ...
        `);
        console.log('Created order_seq table.');
    } catch (e) {
        console.error('Failed to create order_seq:', e);
    } finally {
        conn.release();
    }
})();

// AUTO-MIGRATION: Ensure order_item_audit_logs table exists
(async () => {
    const conn = await pool.getConnection();
    try {
        await conn.query("SELECT id FROM order_item_audit_logs LIMIT 1");
    } catch (e) {
        if (e.code === 'ER_NO_SUCH_TABLE') {
            console.log('Migrating: Creating order_item_audit_logs table...');
            try {
                await conn.query(`
                    CREATE TABLE order_item_audit_logs (
                ...
                `);
                console.log('Created order_item_audit_logs table.');
            } catch (createErr) {
                console.error('Failed to create order_item_audit_logs:', createErr);
            }
        }
    } finally {
        conn.release();
...
router.get('/initial-data', async (req, res) => {
    try {
        ...
        if (global.appCache[cacheKey] && (now - global.appCache[cacheKey].timestamp < cacheTTL)) {
            productPopularity = global.appCache[cacheKey].data;
            // console.log('[POPULARITY] Using cached data');
        } else {
            productPopularity = await safeQuery('product_popularity', 'SELECT product_id, SUM(quantity) as total_qty FROM order_items JOIN orders ON order_items.order_id = orders.id WHERE UPPER(TRIM(orders.status)) = \'COMPLETED\' AND orders.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY) GROUP BY product_id');
            global.appCache[cacheKey] = { data: productPopularity, timestamp: now };
            console.log(`[POPULARITY] Cache updated: ${productPopularity.length} items found.`);
        }

        // Auto-set payment_pending if needed
        ...
        const mappedProducts = products.map(p => {
            ...
            const showInKdsVal = p.show_in_kds !== undefined ? p.show_in_kds : p.showInKds;
            ...
            return {
                ...
                showInKds: !(showInKdsVal === 0 || showInKdsVal === '0' || showInKdsVal === false),
                isCombo: !!p.is_combo,
        ...
        if (orders.length > 0) {
            ...
            orders.forEach(o => {
                ...
                o.payments = payments.filter(p => p.order_id === o.id).map(p => ({
                    ...
                    excessAmount: parseFloat(p.excess_amount || 0),
                }));
            });
        }
...
router.get('/orders/history', async (req, res) => {
    try {
        ...
        if (search) {
            ...
            sql = baseSelect + " AND (o.status = 'completed' OR o.delivery_status = 'delivered')";
...
router.post('/orders', async (req, res) => {
    ...
    try {
        ...
        // --- CALCULAR CORRELATIVO (Contador Atómico vía order_seq) ---
        const seqKey = `date_${getElSalvadorDateStr()}`;
        ...
        let nextDailyOrderNumber = seqRows[0].next_val;

        // Safety check: asegurar que no haya gaps respecto a órdenes existentes
        const [maxRow] = await conn.execute(
            'SELECT COALESCE(MAX(daily_order_number), 0) as max_num FROM orders WHERE branch_id = ? AND DATE(created_at) = CURDATE()',
            [branchId]
        );
        const maxExisting = maxRow[0].max_num;
        if (nextDailyOrderNumber <= maxExisting) {
            console.warn(`[SEQ-FIX] order_seq saltó de ${maxExisting} a ${nextDailyOrderNumber}, corrigiendo a ${maxExisting + 1}`);
            nextDailyOrderNumber = maxExisting + 1;
            await conn.execute(
                'UPDATE order_seq SET next_val = ? WHERE branch_id = ? AND seq_key = ?',
                [nextDailyOrderNumber + 1, branchId, seqKey]
            );
        }

        // Insert Order
        await conn.execute(
            `INSERT INTO orders (id, branch_id, daily_order_number, type, status, kitchen_status, subtotal, tax, discount, manual_discount, delivery_fee, total, created_at, waiter_id, table_id, customer_id, delivery_address_id, amount_paid, change_given, created_by_user_id)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                order.id,
                order.branchId,
        ...
        for (const item of order.items) {
            await conn.execute(
                ...
                [
                    ...
                    item.product.id,
                    item.quantity,
                    item.meat ? item.meat.id : null,
                    item.masaId || (item.masa ? item.masa.id : null),
                    item.total || 0,
                    item.observations || null,
                    item.completed ? 1 : 0,
                    item.comboSelections ? JSON.stringify(item.comboSelections) : null
                ]
            );
    ...
    try {
        ...
        if (updates.payments && Array.isArray(updates.payments) && updates.payments.length > 0) {
            ...
            for (const p of updates.payments) {
                await conn.execute(
                    'INSERT INTO payments (order_id, method, amount, received_by, excess_amount) VALUES (?, ?, ?, ?, ?)',
                    [id, p.method, p.amount, p.receivedBy || null, p.excessAmount || 0]
                );
            }
        }
        ...
        // 4. Fetch FULL POPULATED order for socket (Critical to prevent KDS crash)
        const [dbOrder] = await query('SELECT * FROM orders WHERE id = ?', [id]);
        if (dbOrder) {
            // A. Get Items
            const items = await query('SELECT * FROM order_items WHERE order_id = ?', [id]);
            
            // B. Populated mapping (simulated expansion)
            if (items.length > 0) {
                const itemIds = items.map(i => i.id);
                const placeholder = itemIds.map(() => '?').join(',');
                const allProducts = await query('SELECT * FROM products');
                const allExtras = await query(`
                    SELECT oie.*, pe.name, pe.price 
                    FROM order_item_extras oie 
                    LEFT JOIN product_extras pe ON oie.extra_id = pe.id
                    WHERE oie.order_item_id IN (${placeholder})`, 
                    itemIds
                );
                const allMeats = await query('SELECT * FROM meats');

                dbOrder.items = items.map(i => {
                    ...
                    return {
                        ...
                        completed: !!i.completed,
                        comboSelections: typeof i.combo_selections === 'string' ? JSON.parse(i.combo_selections) : (i.combo_selections || []),
                        product: prod ? { ...prod, price: parseFloat(prod.price) } : null,
                        meat: allMeats.find(m => m.id === i.meat_id) || null,
                        extras: allExtras.filter(e => e.order_item_id === i.id).map(e => ({
                            id: e.extra_id,
                            name: e.name || 'Extra Eliminado',
            ...
            // C. Base fields
            dbOrder.branchId = dbOrder.branch_id;
            dbOrder.kitchenStatus = dbOrder.kitchen_status;
            dbOrder.dailyOrderNumber = dbOrder.daily_order_number;
            dbOrder.total = parseFloat(dbOrder.total);
            dbOrder.subtotal = parseFloat(dbOrder.subtotal);
            dbOrder.deliveryFee = parseFloat(dbOrder.delivery_fee || 0);
            ...
            // Fetch payments
            dbOrder.payments = (await query('SELECT * FROM payments WHERE order_id = ?', [id])).map(p => ({
                ...p,
                amount: parseFloat(p.amount),
                excessAmount: parseFloat(p.excess_amount || 0),
            }));
        ...
        // Emit full populated order
        req.io.emit('order_updated', dbOrder || { ...updates, id });
...
router.post('/audit/item-deletion', async (req, res) => {
    const { orderId, branchId, userId, itemData, reason } = req.body;
    try {
        await query(
            'INSERT INTO order_item_audit_logs (order_id, branch_id, user_id, item_data, reason) VALUES (?, ?, ?, ?, ?)',
            [orderId, branchId, userId, JSON.stringify(itemData), reason || 'Eliminado del carrito']
        );
        res.json({ message: 'Item deletion audited' });
    } catch (err) {
...
router.get('/audit-logs', async (req, res) => {
    try {
        const { startDate, endDate, type } = req.query; // type: 'orders' | 'items' | 'all' (default: orders)
        ...
        if (type === 'items') {
            ...
            const parsedLogs = logs.map(log => ({
                ...log,
                item_data: typeof log.item_data === 'string' ? JSON.parse(log.item_data) : log.item_data
            }));

            res.json(parsedLogs);

        } else {
            // Default: Deleted Orders
            sql = `
                SELECT 
                    al.*, 
                    u.name as deleted_by_name,
...
(async () => {
    ...
    try {
        const columns = [
            ...
            { name: 'last_auto_close_run', type: 'DATETIME DEFAULT NULL' }
        ];
...
router.post('/products', async (req, res) => {
    const { name, price, categoryId, requiresMeat, requiresMasa, availableExtraIds, isActive, isCombo, comboDefinition, showInKds, imageUrl, description } = req.body;
    try {
        const result = await query(
            'INSERT INTO products (name, price, category_id, requires_meat, requires_masa, available_extras, is_active, is_combo, combo_definition, show_in_kds, image_url, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [name, price, categoryId, requiresMeat ? 1 : 0, requiresMasa ? 1 : 0, JSON.stringify(availableExtraIds || []), isActive ? 1 : 0, isCombo ? 1 : 0, JSON.stringify(comboDefinition || null), showInKds !== false ? 1 : 0, imageUrl || null, description || null]
        );
        req.io.emit('catalog_updated');
        res.json({ id: result.insertId, ...req.body });
    } catch (err) {
...
router.put('/products/:id', async (req, res) => {
    ...
    try {
        await query(
            'UPDATE products SET name = ?, price = ?, category_id = ?, requires_meat = ?, requires_masa = ?, available_extras = ?, is_active = ?, is_combo = ?, combo_definition = ?, show_in_kds = ?, image_url = ?, description = ? WHERE id = ?',
            [name, price, categoryId, requiresMeat ? 1 : 0, requiresMasa ? 1 : 0, JSON.stringify(availableExtraIds || []), isActive ? 1 : 0, isCombo ? 1 : 0, JSON.stringify(comboDefinition || null), showInKds !== false ? 1 : 0, imageUrl || null, description || null, id]
        );
        req.io.emit('catalog_updated');
        res.json({ message: 'Updated' });
    } catch (err) {
...
router.post('/cash-closing', async (req, res) => {
    const { branchId, date, initialCash, totalSales, totalCashIn, totalChangeOut, expectedCash, totalOrders, summary } = req.body;
    ...
    try {
        await query(`
            ...
            created_at=NOW()
        `, [branchId, date, initialCash, totalSales, totalCashIn, totalChangeOut, expectedCash, totalOrders || 0, JSON.stringify(summary)]);

        req.io.emit('data_updated');

        // Trigger closing webhook (email) in background
        const reportData = {
            date, initialCash, totalSales, totalCashIn,
            totalChangeOut, expectedCash, totalOrders, summary
        };
        triggerClosingWebhook(branchId, reportData).catch(err =>
            console.error('[CASH-CLOSING] Webhook trigger failed:', err)
        );
        ...
        res.json({
            ...
            totalChangeOut: parseFloat(totalChangeOut),
...
router.post('/admin/clear-data', async (req, res) => {
    const { type, pin, userId, startDate, endDate } = req.body;
        ...
        try {
            ...
            if (type === 'SALES' || type === 'ALL') {
                if (type === 'SALES' && startDate && endDate) {
                    console.log(`[CLEAR-DATA] Clearing Sales Data between ${startDate} and ${endDate}...`);
                    // 1. Get IDs of orders to clear
                    const [ordersToClear] = await conn.query(
                        "SELECT id FROM orders WHERE created_at BETWEEN ? AND ?",
                    ...
                    if (ids.length > 0) {
                        ...
                        await conn.query(`DELETE FROM order_item_extras WHERE order_item_id IN (SELECT id FROM order_items WHERE order_id IN (${idList}))`);
                        await conn.query(`DELETE FROM order_items WHERE order_id IN (${idList})`);
                        await conn.query(`DELETE FROM payments WHERE order_id IN (${idList})`);
                        await conn.query(`DELETE FROM order_audit_logs WHERE order_id IN (${idList})`);
                        await conn.query(`DELETE FROM order_item_audit_logs WHERE order_id IN (${idList})`);
                        await conn.query(`DELETE FROM orders WHERE id IN (${idList})`);
                        console.log(`[CLEAR-DATA] Deleted ${ids.length} orders by date range.`);
                    }
                    // Also clear cash closings for those dates
                    await conn.query(
                        "DELETE FROM cash_closing_reports WHERE date BETWEEN ? AND ?",
                        [startDate, endDate]
                    );
                } else {
                    console.log(`[CLEAR-DATA] Clearing ALL Sales Data...`);
                    ...
                    await conn.query("TRUNCATE TABLE order_item_audit_logs");
            ...
            if (type === 'PRODUCTS' || type === 'ALL') {
                console.log(`[CLEAR-DATA] Clearing Product Data (Type: ${type})...`);
                await conn.query("TRUNCATE TABLE promotions");
                await conn.query("TRUNCATE TABLE product_extras");
                await conn.query("TRUNCATE TABLE meats");
            ...
            if (type === 'SALES' || type === 'ALL') {
                req.io.emit('orders_cleared');
            }

            res.json({ success: true, message: `Data cleared successfully (${type})` });
        } catch (dbErr) {
            await conn.query("SET FOREIGN_KEY_CHECKS = 1");
            throw dbErr;
        } finally {
...
// --- CHECK ORDERS IN RANGE (before clear-data) ---
router.post('/admin/check-orders-range', async (req, res) => {
    const { startDate, endDate, userId } = req.body;
    if (!startDate || !endDate || !userId) {
        return res.status(400).json({ error: 'Missing required parameters (startDate, endDate, userId)' });
    }
    try {
        const [userRows] = await pool.execute(
        ...
        const isSuperAdmin = Array.isArray(roles) && roles.some(r => r.toLowerCase() === 'superadmin');
        if (!isSuperAdmin) {
            return res.status(403).json({ error: 'Only SuperAdmin can check orders range' });
        }
        ...
        console.error('[CHECK-ORDERS-RANGE ERROR]', err);
        res.status(500).json({ error: err.message });
    }
});

// --- BACKUP DATABASE ---
router.post('/admin/backup-database', async (req, res) => {
    const { userId, pin } = req.body;
    if (!userId || !pin) {
        return res.status(400).json({ error: 'Missing required parameters (userId, pin)' });
    }
    try {
        const [userRows] = await pool.execute(
            "SELECT pin, roles FROM users WHERE id = ?",
        ...
        const isSuperAdmin = Array.isArray(roles) && roles.some(r => r.toLowerCase() === 'superadmin');
        if (!isSuperAdmin) {
            return res.status(403).json({ error: 'Only SuperAdmin can backup database' });
        }
        if (String(user.pin) !== String(pin)) {
            return res.status(401).json({ error: 'Invalid PIN' });
        }
        ...
        const cmd = `mysqldump -h ${dbConfig.host} -u ${dbConfig.user} ${dbConfig.password ? `-p${dbConfig.password}` : ''} ${dbConfig.database} > "${dumpPath}"`;
        console.log(`[BACKUP] Creating backup: ${filename}`);
        await execAsync(cmd);

        res.download(dumpPath, filename, async (err) => {
            if (err) {
...
// Used after deploying a new dist. Call with: curl -X POST http://localhost:3001/api/admin/trigger-update
